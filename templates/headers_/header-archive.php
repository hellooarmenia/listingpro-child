...
if( !isset($_GET['s'])){
    ...
    if(isset($_GET['lp_s_cat']) && !empty($_GET['lp_s_cat'])){
        ...
        $termName = apply_filters('lp_results_for_label','Results For').' <strong class="term-name">'.$termo->name.'</strong>';
        $parent = $termo->parent;
    }
    if(isset($_GET['lp_s_cat']) && empty($_GET['lp_s_cat']) && isset($_GET['lp_s_tag']) && !empty($_GET['lp_s_tag'])){
        ...
        $termName = apply_filters('lp_results_for_label','Results For').' <strong>'.$termo->name.'</strong>';
    }
...
<div class="lp-archive-banner <?php echo $emptyFIlter; ?>" style="background-image: url(<?php echo $lp_archive_bg; ?>); ">
    ...
    <div class="lp-header-search archive-search">
        <div class="text-center lp-filter-top-text">
            ...
            <?php }else{ ?>
                <h4 class="lp-title <?php echo $lparchiveBGCLass; ?>"><?php echo apply_filters('lp_results_for_label','Results For');;?> <em><?php echo $termName; ?></em></h4>
            <?php } ?>
        </div>
		<?php 
