...
<header class="header-without-topbar <?php echo esc_attr($headerClass); ?> pos-relative lp-header-full-width">
    <?php if (is_front_page()) { ?>
        <div class="lp-header-overlay"></div> <?php } ?>
    ...
    <div id="menu" class="small-screen">
        ...
        if ($showAddListing == true) {
            ...
            if ( ! empty($addURL)) {
                ...
                <a href="<?php echo listingpro_url('add_listing_url_mode'); ?>"
                   class="lpl-button <?php echo $lpl_add_listing; ?>"><?php esc_html_e('Add Listing',
                        'listingpro'); ?></a>
                <?php
            }
        }
        ...
        if ( ! is_user_logged_in()) {
            ...
            if ($login_popup_style == 'style2') {
                ?>
                <a class="lpl-button lp-right-15 app-view-popup-style"
                   data-target="#app-view-login-popup"><?php esc_html_e('Sign In', 'listingpro'); ?></a>
                <?php
            } else {
                ?>
        ...
        <?php } else { ?>                    <a href="<?php echo wp_logout_url(esc_url(home_url('/'))); ?>"
                                                class="lpl-button" style="right: 10px;"><?php esc_html_e('Sign out ',
                'listingpro'); ?></a>                    <?php } ?>
        <?php
    ...
    <div class="lp-menu-bar header-bg-color-class">
        <div class="<?php echo esc_attr($headerWidth); ?>">
            <div class="row">
                ...
                <?php if ( ! is_front_page()): ?>
                    <div class="col-md-4 col-xs-12"><?php echo do_shortcode('[lp_search_form]'); ?></div>
                <?php endif; ?>
                <div class="header-right-panel clearfix col-md-<?php echo is_front_page() ? 8 : 4; ?> col-sm-10 col-xs-12">
                    <?php
                    if ($headerSrch == 1) {
